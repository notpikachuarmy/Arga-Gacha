...
interface BattleUnit { data:CharacterData|EnemyData; level:number; hp:number; maxHp:number; sprite:Phaser.GameObjects.Image; x:number;y:number; bar:Phaser.GameObjects.Graphics; label:Phaser.GameObjects.Text; classText:Phaser.GameObjects.Text; targetRing?:Phaser.GameObjects.Arc; }
...
class MainScene extends Phaser.Scene{
  ...
  unitPage=0; teamPage=0; codexPage=0; summonPage=0;
  ...
  renderHome(){this.screen='home';this.clear();this.baseBackground();this.header();this.add.image(640,150,'logo').setDisplaySize(310,190);this.txt(640,255,'Mundos distintos. Banners distintos. Una sola colección.',22);this.panel(640,435,780,280,.75);this.button(470,370,300,64,'INVOCAR',()=>this.renderGacha());this.button(810,370,300,64,'EXPEDICIONES',()=>this.renderExpeditions());this.button(470,465,300,64,'FORMAR EQUIPO',()=>this.renderTeam());this.button(810,465,300,64,'VER UNIDADES',()=>this.renderUnits());this.txt(640,555,'v0.4 · Expedición infinita con encuentros persistentes',16);this.nav('home');}
  ...
  pull(count:number){const banner=banners[this.save.selectedBanner%banners.length],cost=banner.ticketCost*count;if(this.save.tickets<cost){this.toast('No tienes tickets suficientes.');return;}this.save.tickets-=cost;const results:CharacterData[]=[];for(let i=0;i<count;i++){const r=Math.random();let rarity=2,acc=0;for(const value of [5,4,3,2]){acc+=banner.rates[String(value)]||0;if(r<=acc){rarity=value;break;}}const pool=characters.filter(c=>c.bannerId===banner.id&&c.rarity===rarity);const c=Phaser.Utils.Array.GetRandom(pool.length?pool:characters.filter(x=>x.bannerId===banner.id));this.save.units.push(starterUnit(c.id));if(!this.save.discovered.includes(c.id))this.save.discovered.push(c.id);results.push(c);}saveGame(this.save);this.summonQueue=results;this.summonIndex=0;this.summonPage=0;this.renderSummon();}
  renderSummon(showAll=false){this.screen='summon';this.clear();this.add.rectangle(640,360,1280,720,0x02050c);this.header('RESULTADOS DE INVOCACIÓN');if(showAll){const per=5,total=Math.ceil(this.summonQueue.length/per),slice=this.summonQueue.slice(this.summonPage*per,this.summonPage*per+per);this.txt(640,98,'RESUMEN DE LA TIRADA',31);slice.forEach((c,i)=>this.drawUnitCard(160+i*240,330,c,200,300,false));this.pageControls(this.summonPage,total,n=>{this.summonPage=n;this.renderSummon(true);},570);this.button(640,635,270,52,'CONTINUAR',()=>this.renderGacha());return;}const c=this.summonQueue[this.summonIndex];const glow=this.add.circle(640,330,220,c.rarity===5?0xffd65a:c.rarity===4?0xb37aff:0x68a7ff,.2);this.tweens.add({targets:glow,scale:1.25,alpha:.05,duration:850,yoyo:true,repeat:-1});const card=this.panel(640,340,470,520,.99,c.rarity===5?0x2b2211:0x101a31).setStrokeStyle(5,c.rarity===5?0xffd65a:c.rarity===4?0xb37aff:0x6f87b1).setInteractive({useHandCursor:true});const p=this.add.image(640,300,`portrait-${c.id}`).setDisplaySize(390,390).setAlpha(0).setScale(.75);this.tweens.add({targets:p,alpha:1,scale:1,duration:430,ease:'Back.easeOut'});this.txt(640,535,c.name,33);this.txt(640,575,`${classMap.get(c.classId)?.name} · ${star(c.rarity)}`,22);this.txt(640,612,`${this.summonIndex+1} / ${this.summonQueue.length}`,16);this.txt(640,660,'PULSA LA CARTA PARA CONTINUAR',15);card.on('pointerdown',(p:Phaser.Input.Pointer)=>{p.event.stopPropagation();if(this.summonIndex<this.summonQueue.length-1){this.summonIndex++;this.renderSummon();}else this.renderSummon(true);});if(this.summonQueue.length>1)this.button(1080,655,225,46,'VER RESUMEN',()=>this.renderSummon(true));}
  ...
  renderUnits(){this.screen='units';this.clear();this.add.rectangle(640,360,1280,720,0x0a1222);this.header('MIS UNIDADES');this.txt(640,95,`UNIDADES · ${this.save.units.length}`,30);const per=12,total=Math.max(1,Math.ceil(this.save.units.length/per));this.unitPage=Math.min(this.unitPage,total-1);this.save.units.slice(this.unitPage*per,this.unitPage*per+per).forEach((u,i)=>{const c=charMap.get(u.characterId)!;const x=135+(i%6)*205,y=205+Math.floor(i/6)*225;const card=this.drawUnitCard(x,y,c,170,190,true,u);card.on('pointerdown',(p:Phaser.Input.Pointer)=>{p.event.stopPropagation();this.showUnit(u,'units');});if(u.favorite)this.txt(x+67,y-80,'♥',20);if(u.locked)this.txt(x-67,y-80,'🔒',15);});this.pageControls(this.unitPage,total,n=>{this.unitPage=n;this.renderUnits();},625);this.nav('units');}
  renderCodex(){this.screen='codex';this.clear();this.add.rectangle(640,360,1280,720,0x0a1222);this.header('CÓDICE MULTIVERSAL');this.txt(640,95,`HOENNIA · ${this.save.discovered.length}/${characters.length} descubiertos`,29);const per=12,total=Math.max(1,Math.ceil(characters.length/per));this.codexPage=Math.min(this.codexPage,total-1);characters.slice(this.codexPage*per,this.codexPage*per+per).forEach((c,i)=>{const known=this.save.discovered.includes(c.id),x=135+(i%6)*205,y=205+Math.floor(i/6)*225;const r=this.add.rectangle(x,y,170,190,known?0x10192c:0x080d18,.98).setStrokeStyle(2,known?0x7f93b7:0x33405a);if(known){r.setInteractive({useHandCursor:true});this.add.image(x,y-30,`portrait-${c.id}`).setDisplaySize(154,125);this.add.rectangle(x,y+56,170,76,0x060a13,.93);this.txt(x,y+42,c.name,15);this.txt(x,y+68,star(c.rarity),14);r.on('pointerdown',(p:Phaser.Input.Pointer)=>{p.event.stopPropagation();this.showCodexCharacter(c);});}else{this.txt(x,y-10,'?',70);this.txt(x,y+60,'SIN DESCUBRIR',13);}});this.pageControls(this.codexPage,total,n=>{this.codexPage=n;this.renderCodex();},625);this.nav('codex');}
  ...
  renderTeam(){this.screen='team';this.clear();this.add.rectangle(640,360,1280,720,0x0a1222);this.header('FORMACIÓN');this.txt(640,92,'EQUIPO ACTUAL',29);for(let i=0;i<3;i++){const u=this.save.units.find(x=>x.uid===this.save.team[i]),x=390+i*250,y=205;const slot=this.add.rectangle(x,y,200,190,0x10192c,.97).setStrokeStyle(3,u?0xf2c45f:0x44516d);if(u){slot.setInteractive({useHandCursor:true}).on('pointerdown',(p:Phaser.Input.Pointer)=>{p.event.stopPropagation();this.save.team=this.save.team.filter(id=>id!==u.uid);saveGame(this.save);this.renderTeam();});const c=charMap.get(u.characterId)!;this.add.image(x,y-25,`portrait-${c.id}`).setDisplaySize(184,125);this.txt(x,y+53,c.name,15);this.txt(x,y+78,`Nv. ${u.level} · pulsa para quitar`,12);}else this.txt(x,y,'HUECO VACÍO',16);}this.txt(640,335,'ELIGE UNA UNIDAD DISPONIBLE',16);const per=14,total=Math.max(1,Math.ceil(this.save.units.length/per));this.teamPage=Math.min(this.teamPage,total-1);this.save.units.slice(this.teamPage*per,this.teamPage*per+per).forEach((u,i)=>{const c=charMap.get(u.characterId)!,selected=this.save.team.includes(u.uid),x=115+(i%7)*177,y=430+Math.floor(i/7)*125;const r=this.add.rectangle(x,y,150,105,selected?0x29375b:0x10192c,.98).setStrokeStyle(2,selected?0xf2c45f:0x7082a4).setInteractive({useHandCursor:true});this.add.image(x-43,y,`portrait-${c.id}`).setDisplaySize(58,88);this.txt(x+27,y-22,c.name,12,.5,82);this.txt(x+27,y+8,`Nv. ${u.level}`,12);this.txt(x+27,y+34,selected?'EN EQUIPO':'AÑADIR',10);r.on('pointerdown',(p:Phaser.Input.Pointer)=>{p.event.stopPropagation();if(selected)this.save.team=this.save.team.filter(id=>id!==u.uid);else if(this.save.team.length<3)this.save.team.push(u.uid);else{this.toast('El equipo ya tiene 3 unidades.');return;}saveGame(this.save);this.renderTeam();});});this.pageControls(this.teamPage,total,n=>{this.teamPage=n;this.renderTeam();},625);this.nav('team');}
  ...
  drawHp(u:BattleUnit){const pct=Phaser.Math.Clamp(u.hp/u.maxHp,0,1);u.bar.clear();u.bar.fillStyle(0x000000,.85).fillRoundedRect(u.x-82,u.y+60,164,18,5);u.bar.fillStyle(pct>.5?0x43c774:pct>.25?0xe0b54c:0xdc5b5b,1).fillRoundedRect(u.x-78,u.y+64,156*pct,10,4);u.label.setText(`${Math.max(0,u.hp)} / ${u.maxHp}`);}
  startBattle(encounter:GeneratedEncounter,replay=false){
    ...
    const blessing=replay?0:encounter.attempts>=5?10:encounter.attempts>=3?5:0;
    const allyBoost=1+blessing/100;
    const allies:BattleUnit[]=selected.map((u,i)=>{const c=charMap.get(u.characterId)!,mult=(1+(u.level-1)*.06)*allyBoost,max=Math.round(c.stats.hp*mult),x=205,y=175+i*170;const sprite=this.add.image(x,y,`char-${c.id}`).setDisplaySize(145,145);const bar=this.add.graphics(),label=this.txt(x,y+87,'',12),classText=this.txt(x,y-88,`${classMap.get(c.classId)?.name} · Nv.${u.level}`,13);this.add.image(x-70,y-88,`class-${c.classId}`).setDisplaySize(26,26);return{data:{...c,stats:{...c.stats,attack:Math.round(c.stats.attack*mult),defense:Math.round(c.stats.defense*mult),hp:max}},level:u.level,hp:max,maxHp:max,sprite,x,y,bar,label,classText};});
    const bads:BattleUnit[]=encounter.enemies.map((foe,i)=>{const e=enemies.find(x=>x.id===foe.enemyId)!,mult=1+(foe.level-1)*.055,max=Math.round(e.stats.hp*mult),x=1075,y=185+i*165;const sprite=this.add.image(x,y,`enemy-${e.id}`).setDisplaySize(145,145).setFlipX(true).setInteractive({useHandCursor:true});const bar=this.add.graphics(),label=this.txt(x,y+87,'',12),classText=this.txt(x,y-88,`${classMap.get(e.classId)?.name} · Nv.${foe.level}`,13);this.add.image(x-70,y-88,`class-${e.classId}`).setDisplaySize(26,26);const unit:BattleUnit={data:{...e,stats:{...e.stats,attack:Math.round(e.stats.attack*mult),defense:Math.round(e.stats.defense*mult),hp:max}},level:foe.level,hp:max,maxHp:max,sprite,x,y,bar,label,classText};unit.targetRing=this.add.circle(x,y,78,0x000000,0).setStrokeStyle(4,0xffd65a).setVisible(false);sprite.on('pointerdown',(p:Phaser.Input.Pointer)=>{p.event.stopPropagation();this.focusedEnemy=unit;for(const b of bads)b.targetRing?.setVisible(b===unit);});return unit;});
    [...allies,...bads].forEach(u=>this.drawHp(u));
    const log=this.add.text(390,535,`Pulsa un enemigo para concentrar ataques.${blessing?` Bendición del Explorador: +${blessing}% a tu equipo.`:''}`,{fontFamily:'Arial',fontSize:'17px',color:'#fff',backgroundColor:'#000c',padding:{x:14,y:10},wordWrap:{width:500}});
    const start=this.button(640,645,320,50,'COMENZAR COMBATE',async()=>{start.r.disableInteractive();start.t.setText('COMBATE EN CURSO');let turn=0;while(allies.some(a=>a.hp>0)&&bads.some(b=>b.hp>0)&&turn<80){turn++;const order=[...allies.filter(x=>x.hp>0),...bads.filter(x=>x.hp>0)].sort((a,b)=>b.data.stats.speed-a.data.stats.speed);for(const unit of order){if(unit.hp<=0)continue;const isAlly=allies.includes(unit),pool=(isAlly?bads:allies).filter(x=>x.hp>0);if(!pool.length)break;const target=isAlly&&this.focusedEnemy&&this.focusedEnemy.hp>0?this.focusedEnemy:Phaser.Utils.Array.GetRandom(pool);await this.attack(unit,target,log);}}
      ...
      if(won){
        const base=this.firstClearRewards(encounter.level);const rewardT=replay?0:base.tickets,rewardF=replay?Math.floor(base.fragments*.5):base.fragments,rewardXp=replay?Math.floor(base.xp*.75):base.xp;
        this.save.tickets+=rewardT;this.save.fragments+=rewardF;selected.forEach(u=>{u.xp+=rewardXp;while(u.xp>=100&&u.level<50){u.level++;u.xp-=100;}});
        if(!replay){this.save.previousEncounter={...encounter,enemies:encounter.enemies.map(e=>({...e}))};this.save.expeditionLevel=encounter.level+1;this.save.currentEncounter=null;}
        saveGame(this.save);log.setText(`¡VICTORIA! +${rewardT} tickets · +${rewardF} fragmentos · +${rewardXp} EXP${replay?' · repetición':' · siguiente nivel desbloqueado'}`);
      }else{
        if(!replay&&this.save.currentEncounter){this.save.currentEncounter.attempts++;saveGame(this.save);}log.setText(replay?'DERROTA EN REPETICIÓN. No pierdes progreso.':'DERROTA. El encuentro queda igual; mejora o cambia el equipo.');
      }
      this.button(640,645,320,50,'VOLVER A EXPEDICIÓN',()=>this.renderExpeditions());
    });
  }
  async attack(attacker:BattleUnit,target:BattleUnit,log:Phaser.GameObjects.Text){const clsA=attacker.data.classId,clsT=target.data.classId,adv=classMap.get(clsA)?.strongAgainst===clsT?1.25:classMap.get(clsT)?.strongAgainst===clsA?0.8:1,damage=Math.max(5,Math.round((attacker.data.stats.attack-target.data.stats.defense*.35)*adv*(.9+Math.random()*.2)));log.setText(`${attacker.data.name} → ${target.data.name}: ${damage}${adv>1?' · ¡VENTAJA!':adv<1?' · resistencia':''}`);await new Promise<void>(resolve=>this.tweens.add({targets:attacker.sprite,x:attacker.x+(target.x>attacker.x?28:-28),duration:90,yoyo:true,onComplete:()=>resolve()}));target.hp-=damage;this.drawHp(target);this.tweens.add({targets:target.sprite,alpha:.25,duration:60,yoyo:true,repeat:1});if(target.hp<=0){target.targetRing?.setVisible(false);if(this.focusedEnemy===target)this.focusedEnemy=null;this.tweens.add({targets:[target.sprite,target.bar,target.label,target.classText],alpha:0,duration:250});}await new Promise<void>(resolve=>this.time.delayedCall(120,()=>resolve()));}
